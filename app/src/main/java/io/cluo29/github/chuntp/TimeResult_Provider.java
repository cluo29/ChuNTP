...
public class TimeResult_Provider extends ContentProvider {
    ...
    public static String AUTHORITY = "io.github.cluo29.chuntp.provider.timeresult";
    ...
    public static final class Time_Result implements BaseColumns {
        ...
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.chuntp.timeresult";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.chuntp.timeresult";
    ...
    public static final String[] TABLES_FIELDS = {
            Time_Result._ID + " integer primary key autoincrement,"
                    ...
                    + Time_Result.DRIFT + " real default 0,"
                    + Time_Result.NTP_TIME + " real default 0"
    };
        ...
        switch (sUriMatcher.match(uri)) {
            case SENSOR_DEV:
                ...
                long accel_id = database.insertWithOnConflict(DATABASE_TABLES[0],
                        Time_Result._ID, values, SQLiteDatabase.CONFLICT_IGNORE);
                database.setTransactionSuccessful();
                database.endTransaction();
                if (accel_id > 0) {
        ...
        switch ( sUriMatcher.match(uri) ) {
            case SENSOR_DEV:
                ...
                for (ContentValues v : values) {
                    long id;
                    try {
                        id = database.insertOrThrow( DATABASE_TABLES[0], Time_Result._ID, v );
                    } catch ( SQLException e ) {
                        id = database.replace( DATABASE_TABLES[0], Time_Result._ID, v );
                    }
                    if( id <= 0 ) {
    ...
    public boolean onCreate() {
        ...
        sensorMap.put(Time_Result.DRIFT, Time_Result.DRIFT);
        sensorMap.put(Time_Result.NTP_TIME, Time_Result.NTP_TIME);
